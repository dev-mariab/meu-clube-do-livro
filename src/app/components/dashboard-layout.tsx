...
export function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-pink-100">
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-pink-200 px-4 py-3 flex items-center justify-between shadow-sm">
      ...
      <aside
        className={`
          ...
          w-64 bg-white/80 backdrop-blur-xl border-r border-pink-200 flex flex-col
          transform transition-transform duration-300 ease-in-out shadow-xl lg:shadow-none
          ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
        `}
      >
        <div className="p-6 border-b border-pink-200 hidden lg:block">
          ...
          <p className="text-sm text-pink-600/80">Suas leituras ✨</p>
        </div>
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${
                  isActive
                    ...
                    : "text-pink-700 hover:bg-pink-100/80"
                }`
              }
            >
        ...
        <div className="p-4 border-t border-pink-200 space-y-3">
          <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-pink-50 to-purple-50 rounded-2xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-pink-600/80 truncate">{user?.email}</p>
            </div>
          </div>
          <Button
            onClick={handleSignOut}
            variant="outline"
            className="w-full justify-start text-pink-600 hover:text-pink-700 hover:bg-pink-50 border-pink-200 rounded-xl"
            size="sm"
          >
